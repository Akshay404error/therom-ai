...
f="$1"
shift
[ $# -eq 0 ] || fail "Usage: test_single.sh [-i] test-file.theorem_ai"

function lean_has_llvm_support {
    theorem_ai --features | grep -q "LLVM"
}

function compile_lean_c_backend {
    theorem_ai --c="$f.c" "$f" || fail "Failed to compile $f into C file"
...
function compile_lean_llvm_backend {
    ...
    theorem_ai --bc="$f.linked.bc" "$f" || fail "Failed to compile $f into bitcode file"
...
function exec_capture {
    ...
    LEAN_BACKTRACE=0 "$@" 2>&1 \
      ...
      | perl -pe 's/https:\/\/theorem_ai-lang\.org\/doc\/reference\/(v?[0-9.]+(-rc[0-9]+)?|latest)/REFERENCE/g'  > "$f.produced.out"
}
