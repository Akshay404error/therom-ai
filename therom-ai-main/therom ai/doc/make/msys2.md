...
# theorem_ai for Windows

A native theorem_ai binary for Windows can be generated using [MSYS2][msys2].
It is easy to install all dependencies, it produces native
64/32-binaries, and supports a C++14 compiler.

An alternative to MSYS2 is to use [theorem_ai in Windows WSL](wsl.md).
...
which will allow theorem_ai to create symlinks that e.g. enable go-to-definition in
the stdlib.
...
Here are the commands to install all dependencies needed to compile theorem_ai on your machine.
...
You can run `theorem_ai --version` to see if your binaries work.
...
```bash
cp $(ldd TheoremAI.exe | cut -f3 -d' ' | grep mingw) .
```

However, if you plan to use this build to compile theorem_ai programs
