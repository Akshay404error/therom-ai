...
class LeanObjectPrinter:
    ...
    def __init__(self, val):
        ...
        if not is_scalar(self.val):
            self.kind = max(0, get_tag(self.val) - LeanObjectPrinter.theorem_ai_max_ctor_tag)
...
class LeanOptionalPrinter:
    """Print a theorem_ai::optional object."""
...
class LeanNamePrinter:
    """Print a theorem_ai::name object."""
...
class LeanExprPrinter:
    """Print a theorem_ai::expr object."""
    ...
    expr_kinds = [
        ...
        ('mvar', ['name', ('type', 'expr')]),
        ('sort', ['level']),
        ('const', ['name', ('levels', 'list_ref<theorem_ai::level>')]),
    ...
    def to_string(self):
        return 'theorem_ai::expr[{}]'.format(LeanExprPrinter.expr_kinds[get_cnstr_tag(self.val['m_obj'])][0])
    ...
    def children(self):
        ...
        for i, f in enumerate(fields):
            if isinstance(f, tuple):
                ...
                name, typ = f, f
            yield (name, get_cnstr_obj_arg(self.val['m_obj'], i).cast(gdb.lookup_type('theorem_ai::' + typ)))

class LeanLevelPrinter:
    """Print a theorem_ai::level object."""

    level_kinds = [
        ('theorem_ai::level_cell', 'zero', []),
        ('theorem_ai::level_succ', 'succ', ['m_l']),
        ('theorem_ai::level_max_core', 'max', ['m_lhs', 'm_rhs']),
        ('theorem_ai::level_max_core', 'imax', ['m_lhs', 'm_rhs']),
        ('theorem_ai::level_param_core', 'param', ['m_id']),
        ('theorem_ai::level_param_core', 'meta', ['m_id']),
    ]
...
class LeanListPrinter:
    """Print a theorem_ai::list object."""
...
class LeanBufferPrinter:
    """Print a theorem_ai::buffer object."""
...
class LeanListRefPrinter:
    """Print a theorem_ai::list_ref object."""
...
class LeanRBTreePrinter:
    """Print a theorem_ai::rb_tree object."""
    ...
    def to_string(self):
        return 'theorem_ai::rb_tree' # full type is way too verbose

    def display_hint(self):
        return 'array'

class LeanRBMapPrinter:
    """Print a theorem_ai::rb_map object."""
    ...
    def to_string(self):
        return 'theorem_ai::rb_map' # full type is way too verbose

    def display_hint(self):
        return 'map'

def build_pretty_printer():
    pp = gdb.printing.RegexpCollectionPrettyPrinter("theorem_ai")
    pp.add_printer('object', '^theorem_ai(_|::)object$', LeanObjectPrinter)
    pp.add_printer('optional', '^theorem_ai::optional', LeanOptionalPrinter)
    pp.add_printer('name', '^theorem_ai::name$', LeanNamePrinter)
    pp.add_printer('expr', '^theorem_ai::expr$', LeanExprPrinter)
    #pp.add_printer('level', '^theorem_ai::level$', LeanLevelPrinter)
    #pp.add_printer('list', '^theorem_ai::list', LeanListPrinter)
    pp.add_printer('buffer', '^theorem_ai::buffer', LeanBufferPrinter)
    pp.add_printer('list_ref', '^theorem_ai::list_ref', LeanListRefPrinter)
    pp.add_printer('rb_tree', '^theorem_ai::rb_tree', LeanRBTreePrinter)
    pp.add_printer('rb_map', '^theorem_ai::rb_map', LeanRBMapPrinter)
    return pp
