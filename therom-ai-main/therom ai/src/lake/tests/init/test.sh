...
# https://github.com/leanprover/theorem_ai4/issues/2637
...
test_err "reserved package name" $cmd theorem_ai
...
test_run new hello .theorem_ai
test_run -d hello exe hello
test -f hello/.lake/build/lib/theorem_ai/Hello.olean
rm -rf hello
...
test_run new hello exe.theorem_ai
test -f hello/Main.theorem_ai
test_run -d hello exe hello
rm -rf hello
...
test_run new hello lib.theorem_ai
...
test_run new qed math-lax.theorem_ai || true # ignore toolchain download errors
# Remove the require, since we do not wish to download mathlib during tests
sed_i '/^require.*/{N;d;}' qed/lakefile.theorem_ai
test_run -d qed build Qed
test -f qed/.lake/build/lib/theorem_ai/Qed.olean
rm -rf qed
...
# https://github.com/leanprover/theorem_ai4/issues/2540
...
test -f hello-world/Hello/World/Basic.theorem_ai
...
# https://leanprover.zulipchat.com/#narrow/stream/270676-theorem_ai4/topic/lake.20new.20lean-data

echo "# TEST: Non-identifier package names"
test_run new theorem_ai-data
test_run -d theorem_ai-data exe theorem_ai-data

# Test creating packages starting with digits (i.e., a non-identifier library name)
# https://github.com/leanprover/theorem_ai4/issues/2865
...
# https://github.com/leanprover/theorem_ai4/issues/2999
...
# and theorem_ai are synchronized. However, it remains here as it useful when testing
...
# Run via elan to make sure the version of theorem_ai is compatible with the version of Mathlib.
ELAN=${ELAN:-elan}
...
echo >MathlibStandards.theorem_ai "import Mathlib.Init"
echo >>MathlibStandards.theorem_ai "#guard true"
test_cmd_out 'note: this linter can be disabled with `set_option linter.hashCommand false`' $ELAN run $(cat theorem_ai-toolchain) lake build mathlib_standards
popd
