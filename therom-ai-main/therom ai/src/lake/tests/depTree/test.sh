...
$LAKE new b lib.theorem_ai
pushd b
git init
git checkout -b master
cat >>lakefile.theorem_ai <<EOF
...
sed_i 's/master/init/g' lakefile.theorem_ai
...
# a@3
pushd a
echo '-- third commit in a' >>A.theorem_ai
...
grep --color 'third commit in a' .lake/packages/a/A.theorem_ai && exit 1 || true
git diff --exit-code
popd
...
pushd b
# b@2: a@master -> a@main
sed_i 's/master/main/' lakefile.theorem_ai
$LAKE update a -v
git commit -am 'second commit in b'
popd
pushd a
# a@4/main
sed_i 's/third commit/fourth commit/' A.theorem_ai
git commit -am 'fourth commit in a'
popd
pushd c
# c@2
echo '-- second commit in c' >>C.theorem_ai
...
grep --color 'second commit in c' .lake/packages/c/C.theorem_ai && exit 1 || true
# test 119: pickup a@3 and not a@4
grep --color 'third commit in a' .lake/packages/a/A.theorem_ai
# test the removal of `c` from the manifest
grep --color "\"c\"" lake-manifest.json
sed_i '/require c/d' lakefile.theorem_ai
$LAKE update c -v
