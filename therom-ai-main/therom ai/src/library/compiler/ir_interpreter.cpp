...
namespace ir {
// C++ wrappers of theorem_ai data types
...
private:
    ...
    void check_system() {
        try {
            theorem_ai::check_system("interpreter");
    ...
    /** \brief Retrieve theorem_ai declaration from elab_environment. */
        ...
        if (e.m_native.m_addr) {
            ...
            if (decl_tag(e.m_decl) == decl_kind::Extern) {
                ...
                throw exception(sstream() << "Could not find native implementation of external declaration '" << fn
                                          ...
                                          << "For declarations from `Init`, `Std`, or `theorem_ai`, you need to set `supportInterpreter := true` "
                                          << "in the relevant `lean_exe` statement in your `lakefile.theorem_ai`.");
            }
...
public:
    ...
    uint32 run_main(list_ref<string_ref> const & args) {
        ...
        if (io_result_is_ok(w)) {
            int ret = 0;
            theorem_ai::expr ret_ty = m_env.get("main").get_type();
            if (is_arrow(ret_ty)) {
