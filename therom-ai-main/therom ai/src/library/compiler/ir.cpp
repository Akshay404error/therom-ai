        ...
        while (is_let(e)) {
            ...
            if (is_fvar(val) || is_enf_neutral(val)) {
                ...
                if (is_llnf_sset(op) || is_llnf_fset(op) || is_llnf_f32set(op) || is_llnf_uset(op)) {
                    /* In the theorem_ai IR, sset and uset are instructions that perform destructive updates. */
...
/*
@[export TheoremAI.ir.compile_core]
...
extern "C" LEAN_EXPORT object* lean_add_extern(object * env, object * fn) {
    try {
        ...
        // throw; // We use to uncomment this line when debugging weird bugs in the theorem_ai/C++ interface.
