...
/*
A constructor object contains a header, then a sequence of pointers to other theorem_ai objects,
...
class to_lambda_pure_fn {
    ...
    optional<unsigned> is_enum_type(expr const & type) {
        return ::theorem_ai::is_enum_type(env(), type);
    }

    unsigned get_arity(name const & n) const {
        return ::theorem_ai::get_llnf_arity(env(), n);
    }
    ...
    expr ensure_terminal(expr const & e) {
        ...
        if (is_cases_on_app(env(), e) || is_fvar(e) || is_join_point_app(e) || is_enf_unreachable(e)) {
            ...
            return ::theorem_ai::mk_let("_res", type, e, mk_bvar(0));
        }
    }
    ...
    cnstr_info get_cnstr_info(name const & n) {
        ...
        if (it != m_cnstr_info_cache.end())
            return it->second;
        cnstr_info r = ::theorem_ai::get_cnstr_info(m_st, n);
    ...
    expr visit_constructor(expr const & e) {
        ...
        if (optional<unsigned> r = ::theorem_ai::is_enum_type(env(), I)) {
