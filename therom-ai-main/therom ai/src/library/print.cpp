...
struct print_expr_fn {
    ...
    static bool is_atomic(expr const & a) {
        if (::theorem_ai::is_atomic(a)) return true;
    ...
    static bool is_arrow(expr const & t) {
        return theorem_ai::is_arrow(t) && binding_info(t) == binder_info::Default;
    }
    ...
    void print(expr const & a) {
        ...
        case expr_kind::Lit:
            ...
            case literal_kind::String: {
                ...
                out() << "\"" << escaped(val.c_str()) << "\""; break; // HACK theorem_ai string as C string
            }}
            break;
        }
    }
