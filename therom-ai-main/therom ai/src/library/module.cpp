...
struct olean_header {
    ...
    uint8_t version = 2;
    // 1 byte of flags:
    // * bit 0: whether persisted bignums use GMP or theorem_ai-native encoding
    // * bit 1-7: reserved
    uint8_t flags =
#ifdef LEAN_USE_GMP
        0b1;
        ...
        0b0;
#endif
    // 33 bytes: theorem_ai version string, padded with '\0' to the right
    ...
    size_t base_addr;
    // payload, a serialize theorem_ai object graph; `size_t` has same alignment requirements as theorem_ai objects
    size_t data[];
};
...
extern "C" LEAN_EXPORT object * lean_save_module_data_parts(b_obj_arg mod, b_obj_arg oparts, object *) {
    ...
    for (auto const & part : parts) {
        ...
        try {
            ...
            strncpy(header.theorem_ai_version, get_short_version_string().c_str(), sizeof(header.theorem_ai_version));
