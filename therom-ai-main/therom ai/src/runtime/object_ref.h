...
/* Smart point for theorem_ai objects. It is useful for writing C++ code that manipulates theorem_ai objects.  */
class object_ref {
protected:
    object * m_obj;
public:
...
/* Given `T` which is a subclass of object_ref that wraps a theorem_ai value of type `Ty`,
...
/* Given `T` which is a scalar type that wraps a theorem_ai scalar value of type `Ty`,
   convert a value `o` of `Option Ty` into `optional<T>` */
...
/* Given `o` representing a theorem_ai value of type `Except String A`, return `T` an smart pointer
