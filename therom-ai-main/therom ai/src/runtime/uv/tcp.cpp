...
extern "C" LEAN_EXPORT lean_obj_res lean_uv_tcp_recv(b_obj_arg socket, uint64_t buffer_size, obj_arg /* w */) {
    ...
    int result = uv_read_start((uv_stream_t*)tcp_socket->m_uv_tcp, [](uv_handle_t* handle, size_t suggested_size, uv_buf_t* buf) {
        ...
        if (nread >= 0) {
            ...
            lean_promise_resolve(mk_except_ok(theorem_ai::mk_option_some(byte_array)), promise);
        } else if (nread == UV_EOF) {
            lean_dec(byte_array);
            lean_promise_resolve(mk_except_ok(theorem_ai::mk_option_none()), promise);
        } else if (nread < 0) {
            lean_dec(byte_array);
...
extern "C" LEAN_EXPORT lean_obj_res lean_uv_tcp_new(obj_arg /* w */) {
    lean_always_assert(
        false && ("Please build a version of theorem_ai4 with libuv to invoke this.")
    );
}
