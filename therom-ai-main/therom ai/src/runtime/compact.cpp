...
struct object_compactor::max_sharing_table {
    theorem_ai::unordered_set<max_sharing_key, max_sharing_hash, max_sharing_eq> m_table;
...
/*
  Remark: g_null_offset must NOT be a valid theorem_ai scalar value (e.g., static_cast<size_t>(-1)).
  Recall that theorem_ai scalar are odd size_t values. So, we use (static_cast<size_t>(-1) - 1) which is an even number.
