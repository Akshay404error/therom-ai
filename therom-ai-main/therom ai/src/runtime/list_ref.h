...
/* Wrapper for manipulating theorem_ai lists in C++ */
template<typename T>
...
public:
    ...
    friend bool operator==(list_ref const & l1, list_ref const & l2) {
        ...
        while (!is_scalar(it1) && !is_scalar(it2)) {
            if (it1 == it2)
                return true;
            T const & h1 = ::theorem_ai::head<T>(it1);
            T const & h2 = ::theorem_ai::head<T>(it2);
            if (h1 != h2)
                return false;
            it1 = cnstr_get(it1, 1);
            it2 = cnstr_get(it2, 1);
    ...
    public:
        ...
        T const & operator*() { return ::theorem_ai::head<T>(m_it); }
    };
