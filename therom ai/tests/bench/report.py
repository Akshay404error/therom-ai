...
CATBAG = {
    '.theorem_ai': ("theorem_ai", "etime"),
    '.no_reuse.theorem_ai': ("-reuse", "etime"),
    '.no_borrow.theorem_ai': ("-borrow", "etime"),
    '.no_st.theorem_ai': ("-ST", "etime"),
    '.gcc.theorem_ai': ("theorem_ai+GCC9", "etime"),
    '.gc.theorem_ai': ("theorem_ai del", "gc"),
    '.TheoremAI.perf': ("CM", "cache-misses"),
    '.hs': ("GHC", "etime"),
    '.gc.hs': ("GHC GC", "gc"),
...
for sfx in ['', '_rbmap']:
    ...
    for bench in benches:
        if sfx == '' and bench.startswith('rbmap_') or sfx == '_rbmap' and not bench.startswith('rbmap'):
            continue
        norm = single('rbmap_1' if bench.startswith('rbmap') and sfx == '_rbmap' else bench, '.theorem_ai', 'etime').mean()
