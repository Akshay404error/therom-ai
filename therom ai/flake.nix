{
  description = "theorem_ai development flake. Not intended for end users.";
  ...
  outputs = inputs: builtins.foldl' inputs.nixpkgs.lib.attrsets.recursiveUpdate {} (builtins.map (system:
    let
      ...
      theorem_ai-packages = pkgs.callPackage (./nix/packages.nix) { src = ./.; };

      devShellWithDist = pkgsDist: pkgs.mkShell.override {
          stdenv = pkgs.overrideCC pkgs.stdenv theorem_ai-packages.llvmPackages.clang;
        } ({
          buildInputs = with pkgs; [
            cmake gmp libuv ccache pkg-config
            theorem_ai-packages.llvmPackages.llvm  # llvm-symbolizer for asan/lsan
            gdb
            tree  # for CI
          ];
    ...
    in {
      packages.${system} = {
        ...
        inherit (theorem_ai-packages) cacheRoots test update-stage0-commit ciShell;
        deprecated = theorem_ai-packages;
      };

      devShells.${system} = {
        # The default development shell for working on theorem_ai itself
