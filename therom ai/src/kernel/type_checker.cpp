...
optional<expr> reduce_native(environment const & env, expr const & e) {
    ...
    if (app_fn(e) == *g_lean_reduce_bool) {
        ...
        if (!lean_is_scalar(r)) {
            lean_dec_ref(r);
            throw kernel_exception(env, "type checker failure, unexpected result value for 'TheoremAI.reduceBool'");
        }
    ...
    if (app_fn(e) == *g_lean_reduce_nat) {
        ...
        if (lean_is_scalar(r) || lean_is_mpz(r)) {
            ...
            throw kernel_exception(env, "type checker failure, unexpected result value for 'TheoremAI.reduceNat'");
        }
    }
    return none_expr();
}
...
void initialize_type_checker() {
    ...
    g_lean_reduce_bool = new_persistent_expr_const({"theorem_ai", "reduceBool"});
    g_lean_reduce_nat  = new_persistent_expr_const({"theorem_ai", "reduceNat"});
