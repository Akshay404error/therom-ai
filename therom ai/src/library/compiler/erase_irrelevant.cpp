...
class erase_irrelevant_fn {
    ...
    optional<unsigned> has_trivial_structure(name const & I_name) {
        return ::theorem_ai::has_trivial_structure(env(), I_name);
    }

    expr mk_runtime_type(expr e) {
        return ::theorem_ai::mk_runtime_type(m_st, m_lctx, e);
    }
    ...
    expr visit_lambda_core(expr e, bool is_minor) {
        ...
        if (is_minor && is_lambda(r)) {
            ...
            r = ::theorem_ai::mk_let("_x", mk_enf_object_type(), r, mk_bvar(0));
        }
    ...
    expr elim_nat_cases(buffer<expr> & args) {
        ...
        minor_s          = ::theorem_ai::mk_let(next_name(), nat_type, pred, binding_body(minor_s));
    ...
    expr elim_int_cases(buffer<expr> & args) {
        ...
        expr minor_p     = args[2];
        minor_p          = ::theorem_ai::mk_let(next_name(), nat_type, abs, binding_body(minor_p));
        minor_p          = visit_minor(minor_p);
        expr one         = mk_lit(literal(nat(1)));
        expr minor_n     = args[3];
        minor_n          = ::theorem_ai::mk_let(next_name(), nat_type, abs,
                           ::theorem_ai::mk_let(next_name(), nat_type, mk_app(mk_constant(get_nat_sub_name()), mk_bvar(0), one),
    ...
    expr elim_array_cases(buffer<expr> & args) {
        ...
        return
            ::theorem_ai::mk_let(next_name(), mk_enf_object_type(), mk_app(mk_constant(get_array_to_list_name()), mk_enf_neutral(), major),
                           binding_body(minor));
    }
    ...
    expr elim_float_array_cases(buffer<expr> & args) {
        ...
        lean_always_assert(is_lambda(minor));
        return
            ::theorem_ai::mk_let(next_name(), mk_enf_object_type(), mk_app(mk_constant(get_float_array_data_name()), major),
                           binding_body(minor));
    }
    ...
    expr elim_byte_array_cases(buffer<expr> & args) {
        ...
        lean_always_assert(is_lambda(minor));
        return
            ::theorem_ai::mk_let(next_name(), mk_enf_object_type(), mk_app(mk_constant(get_byte_array_data_name()), major),
                           binding_body(minor));
    }
    ...
    expr elim_uint_cases(name const & uint_name, buffer<expr> & args) {
        ...
        lean_always_assert(is_lambda(minor));
        return
          ::theorem_ai::mk_let(next_name(), mk_enf_object_type(), mk_app(mk_const(name(uint_name, "toNat")), major),
                         binding_body(minor));
    }
    ...
    expr mk_let(unsigned saved_fvars_size, expr r) {
        ...
        while (i > saved_fvars_size) {
            ...
            r      = ::theorem_ai::mk_let(std::get<0>(m_let_entries[i]), std::get<1>(m_let_entries[i]), v, r);
        }
