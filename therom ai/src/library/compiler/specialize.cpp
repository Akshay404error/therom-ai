...
bool is_class(elab_environment const & env, expr type) {
    // This is a temporary hack. We do not unfold `type` here, but we should. We will fix it when we reimplement the compiler in TheoremAI.
    while (is_pi(type)) {
...
class specialize_fn {
    ...
    bool has_specialize_attribute(name const & fn) {
        return ::theorem_ai::has_specialize_attribute(env(), fn) || m_to_respecialize.contains(fn);
    }
        ...
        void collect_fvar(expr const & x, bool in_binder) {
            ...
            if (!in_binder) {
                ...
                /* HACK to avoid work duplication.
                   ...
                   TODO: implement the real fix when we re-implement the code in TheoremAI.
                 */
    ...
    void sort_fvars(buffer<expr> & fvars) {
        ::theorem_ai::sort_fvars(m_lctx, fvars);
    }
    ...
    expr abstract_spec_ctx(spec_ctx const & ctx, expr const & code) {
        ...
        while (i > 0) {
            ...
            if (val && !letvars_in_params.contains(fvar_name(fvars[i]))) {
                r = ::theorem_ai::mk_let(decl.get_user_name(), type, abstract(*val, i, fvars), r);
            } else {
                r = ::theorem_ai::mk_lambda(decl.get_user_name(), type, r, decl.get_info());
            }
        }
        return r;
    }
    ...
    optional<comp_decl> mk_new_decl(comp_decl const & pre_decl, buffer<expr> const & fvars, buffer<expr> const & fvar_vals, spec_ctx & ctx) {
        ...
        for (unsigned i = 0; i < fvars.size(); i++) {
            ...
            try {
                ...
                /* We may fail to infer the type of fvar_vals, since it may be recursive
                   This is a workaround. When we re-implement the compiler in theorem_ai,
        ...
        if (has_fvar(code)) {
            /* This is yet another temporary hack. It addresses an assertion violation triggered by test 1293.theorem_ai for issue #1293 */
            return optional<comp_decl>();
        }
    ...
    optional<expr> specialize(expr const & fn, buffer<expr> const & args, spec_ctx & ctx) {
        ...
        if (num_parts(const_name(fn)) > 32) {
            ...
            // We need to move the code to theorem_ai ASAP.
            return none_expr();
        }
        ...
        /* When `m_params.size > 0`, it is not safe to reuse cached specialization, because we don't know at reuse time
           ...
           See test `tests/theorem_ai/run/specbug.theorem_ai`.
           ...
           TODO: when we reimplement this module in theorem_ai, we should have a better caching heuristic. */
