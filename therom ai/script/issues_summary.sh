...
# Fetch only open issues (excluding PRs and closed issues) from the repository 'leanprover/theorem_ai4'
issues=$(gh api repos/leanprover/theorem_ai4/issues --paginate --jq '.[] | select(.pull_request == null and .state == "open") | {url: .html_url, title: .title, created_at: (.created_at | split("T")[0]), updated_at: (.updated_at | split("T")[0]), number: .number, assignee: (.assignee.login // ""), labels: [.labels[].name] | join(",")}')
...
echo "$issues" | while IFS= read -r issue; do
    ...
    reaction_data=$(gh api repos/leanprover/theorem_ai4/issues/$issue_number/reactions --paginate --jq 'length' 2>&1)
