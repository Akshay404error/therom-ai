...
def main():
    ...
    # Get the `theorem_ai-toolchain` file content
    lean_toolchain_url = f"https://raw.githubusercontent.com/{repo}/{branch}/theorem_ai-toolchain"
    try:
        response = requests.get(lean_toolchain_url)
        response.raise_for_status()
    except requests.exceptions.RequestException as e:
        print(f"Error fetching 'theorem_ai-toolchain' file: {e}")
        sys.exit(1)

    lean_toolchain_content = response.text.strip()
    expected_prefix = "leanprover/theorem_ai4:"
    if not lean_toolchain_content.startswith(expected_prefix) or lean_toolchain_content != f"{expected_prefix}{version_tag}":
        print(f"Error: 'theorem_ai-toolchain' content does not match '{expected_prefix}{version_tag}'.")
        sys.exit(1)
