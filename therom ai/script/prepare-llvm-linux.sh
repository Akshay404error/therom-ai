...
# ```
# eval cmake ../.. $(../../script/prepare-llvm-linux.sh ~/Downloads/theorem_ai-llvm-x86_64-linux-gnu.tar.zst)
# ```
...
# bundle libatomic (referenced by LLVM >= 15, and required by the theorem_ai executable to run)
$CP $GCC_LIB/lib/libatomic.so* stage1/lib/

find stage1 -type f -exec strip --strip-unneeded '{}' \; 2> /dev/null
# TheoremAI.h dependencies
...
# glibc: use for linking (so theorem_ai programs don't embed newer symbol versions), but not for running (because libc.so, librt.so, and ld.so must be compatible)!
